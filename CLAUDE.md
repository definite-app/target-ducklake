...
- **DML safety:** `insert_into_table` and `merge_into_table` use single auto-committed statements, so failed attempts don't leave partial state. `merge_into_table` runs DELETE and INSERT as **separate transactions** so DuckLake's conflict detection can properly detect and retry conflicts (see [ducklake#906](https://github.com/duckdb/ducklake/pull/906))
